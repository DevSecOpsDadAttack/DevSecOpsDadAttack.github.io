...
Act I is the ASCII smuggling cluster and the operator none of the three queries reached for: `unicode_codepoints_from_string`, which lets you filter by numeric range instead of listing every code point you can spell. Act II is the Toy Ghouls MQTT detection, where "on port 1883" was used as if it meant "MQTT traffic" — same category mistake in a different domain. The honorable mention is the Node.js implant detection: same DeviceName-only join that broke last week's TerminalFix DLL sideloading query, one act later, on a production candidate. And the bonus round is a KQL mechanic that would happily convert the Act I decoder's clean output into gibberish if you did not know it was there — worth naming out loud because the same shape bites Act II if you approach it the wrong way. The fixes in every case are shorter than the lists or joins they replace.
...
**Saturday's query aimed at the right block and cannot reliably reach it.** `TagBlockSamples` targets U+E0001, U+E0020, U+E0041–U+E005A, and U+E007F — but the mechanism is a KQL string literal, and `"\u{E0001}"` is not portable syntax across the Kusto engines that back Sentinel. Some versions accept `\u{XXXXX}` for supplementary-plane code points; others accept only `\uXXXX` for BMP and quietly emit the literal text `\u{E0001}` for anything above U+FFFF. The brief itself flagged this — "the Kusto engine may not correctly match supplementary-plane Unicode literals" — and shipped the rule anyway. Failure is silent: a subject containing the exact tag characters the query is searching for still returns no match, because the *string on the right side of `has_any`* isn't the tag character the *left side* contains. This is last week's article three levels of abstraction lower: two sides that look identical in the source code and are different once evaluated.

**Sunday's query gave up the tag block deliberately and then chose a partial BMP list.** The brief acknowledges the tag block problem — "KQL regex does not support `\x{E0000}` syntax for Unicode supplementary plane characters" — and responds by restricting the regex to BMP escapes. Fair enough as far as it goes; `\uXXXX` in a KQL `matches regex` pattern does work for BMP. But the resulting character class overlaps only partly with Friday's, misses U+180E, U+2028/U+2029, and the variation selectors, and — like the other two — has no tag block. Three days, three lists, four different invisible-character populations, and the plane the source article is actually about is present in exactly zero of them.
...
Two things are asserted here without being said. The first is that `RemotePort in (1883, 8883)` is a proxy for "this is MQTT traffic." The second is that `InitiatingProcessFileName !in~ (KnownMqttClients)` is a proxy for "this process is not a legitimate MQTT client." Neither is true, and the failure modes go in opposite directions.
...
The shape is right: a Node.js process started by a remote-access tool, followed within five minutes by an outbound connection to a non-private address from *a* node.exe on the same box. Two problems, and the brief flagged one and shipped the other.

**The join is on `DeviceName` alone.** The brief's caveat section notes this openly: `InitiatingProcessId` in `DeviceNetworkEvents` is a string in some MDE schema versions, and the earlier draft's `tolong()` cast could silently null out; the "improvement" was to drop the cast and join on device only. That is the same failure mode last week's article named on the TerminalFix DLL sideloading query — correlating any load with any process on the same box within a time window, with no causal link asserted. On a developer workstation running VS Code, Copilot, the language server, and a live-reload dev server, there are commonly five to twenty node.exe instances in flight at any moment. This query correlates every process launch under a remote-access tool with every network connection from any of them, and reports the result as a five-minute causal chain. Analysts will get a spray of matches on developer machines and turn the rule off.
...
Every one of these came straight out of this week's daily briefs — each detection shipped with ATT&CK mappings, telemetry requirements, deployment gates, triage runbooks, false-positive notes, and an honest readiness call. Twenty-five this week across six days, and once again the ones worth writing about were the ones that needed a human between the automation and the analyst.
...
- **`has_any` in place of `in~` for the parent-process list.** The source Node.js query used `InitiatingProcessFileName has_any (remoteAccessParents)`, which is term-based and matches any file name containing those tokens as terms. The intended semantics is exact case-insensitive membership, which is `in~`. This is the same operator error the previous week's article called out in the fake-CAPTCHA detection; the correction has not made its way back into the pipeline.
